...
class CashbackRule(models.Model):
    ...
    state = fields.Selection([('draft', 'Draft'), ('waiting', 'Waiting Approval'),('approve', 'Running'), ('cancel', 'Cancelled')],
    ...
    def compute_revenue(self, group_id, cid, frequency):
        ...
        cr.execute("""SELECT COALESCE(SUM(ai.amount_total), 0) as revenue FROM account_move ai
            inner join res_partner rpc on rpc.id = ai.partner_id
            inner join master_customer_cashback mcc on mcc.group_id = rpc.group_id 
            where ai.state in ('draft','posted') and mcc.group_id = {0} and mcc.company_id = {1} and ai.company_id = {1} 
            and mcc.state = 'confirm' and ai.invoice_date between '{2}' and '{3}'""".format(group_id, cid, dfrom, dto))
        # ('open','paid')
        rev = cr.dictfetchall()
        cr.execute("""SELECT ai.partner_id,ai.id FROM account_move ai
            inner join res_partner rpc on rpc.id = ai.partner_id
            inner join master_customer_cashback mcc on mcc.group_id = rpc.group_id 
            where ai.state in ('draft','posted') and mcc.group_id = {0} and mcc.company_id = {1} and ai.company_id = {1} 
            and mcc.state = 'confirm' and ai.invoice_date between '{2}' and '{3}'""".format(group_id, cid, dfrom, dto))
        account =  cr.dictfetchall()
        revenue = math.ceil(rev[0]['revenue']) if rev else 0.0
        if status not in ['month', 'wilayah']:
        ...
        cr.execute("""SELECT COALESCE(sum(ctl.amount), 0) as target FROM adireksa_customer_target ct
            ...
            where ct.id = {0} and ct.company_id = {1} and ct.state = 'approve' {2}""".format(group_id, cid, whr))
        tgt = cr.dictfetchone()
        target = tgt['target'] if tgt else 0.0
        return revenue, target, dto, status,account ,dfrom,dto

    def get_auto_cashback_ref(self, status,dfrom,dto):
        year = dfrom.replace('-',' ').split()[0]
        if status == 'q1':
            title = 'Quarter 1'
            ...
            title = dfrom
            month_range = dto
        return title, month_range,year
    
    def check_frequency_percent_value(self, data, group_id, frequency, revenue, target, status ,dfrom,dto):
        cr = self.env.cr
        total = 0.0
        ref = ''
        line_id = []
        data_line = data.line_level_id
        percent_range = data_line.mapped('percent_hit')
        title, month_range ,year  = self.get_auto_cashback_ref(status,dfrom,dto)
        percent = (revenue /target ) * 100
        if percent not in percent_range and revenue >=target:
            percent_range.append(percent)
            percent_range.sort()
            index = percent_range.index(percent)
            if index != 0 :
                used_range = percent_range[index - 1]
                data_used = data_line.filtered(lambda x:x.percent_hit == used_range)
                total = (data_used.formula / 100) * (revenue - target)
                ref = 'Omzet Periode %s (%s) Tahun %s Hit Target, Total Invoice: Rp. %s' % (
                title, month_range, year,'{:0,.0f}'.format(revenue))
                line_id.append(data.id)
        return math.ceil(total), ref, line_id
                
    def check_frequency_amount_value(self, data, group_id, frequency, revenue, target, status ,dfrom,dto):
        cr = self.env.cr
        total = 0.0
        ref = ''
        line_id = []
        data_line = data.line_level_id
        amount_range = data_line.mapped('amount_hit')
        title, month_range ,year  = self.get_auto_cashback_ref(status,dfrom,dto)
        amount = revenue
        if amount not in amount_range and revenue >=target and self.env.company.id == 1:
            amount_range.append(amount)
            amount_range.sort()
            index = amount_range.index(amount)
            if index != 0 :
                used_range = amount_range[index - 1]
                data_used = data_line.filtered(lambda x:x.amount_hit == used_range)
                total = (data_used.formula / 100) * (revenue - target)
                ref = 'Omzet Periode %s (%s) Tahun %s Hit Target, Total Invoice: Rp. %s' % (
                title, month_range, year,'{:0,.0f}'.format(revenue))
                line_id.append(data.id)
        elif self.env.company.id == 2 :
            if amount not in amount_range :
                amount_range.append(amount)
                amount_range.sort()
                index = amount_range.index(amount)
                if index != 0 :
                    used_range = amount_range[index - 1]
                    data_used = data_line.filtered(lambda x:x.amount_hit == used_range)
                    total = (data_used.formula / 100) * (revenue)
                    ref = 'Omzet Periode %s (%s) Tahun %s Hit Target, Total Invoice: Rp. %s' % (
                    title, month_range, year,'{:0,.0f}'.format(revenue))
                    line_id.append(data.id)
        return math.ceil(total), ref, line_id
            
    def check_frequency_revenue_value(self, data, group_id, frequency, revenue, target, status ,dfrom,dto):
        cr = self.env.cr
        total = 0.0
        ref = ''
        line_id = []
    ...
    def check_frequency_invoice_value(self, data, group_id, frequency, revenue, target, status ,dfrom,dto):
        cr = self.env.cr
        total = 0.0
        ref = ''
        line_id = []
    ...
    def check_frequency_wilayah_value(self, data, group_id, frequency, revenue, target, status ,dfrom,dto):
        cr = self.env.cr
        total = 0.0
        ref = ''
        line_id = []
        ...
        cr.execute("""SELECT COALESCE(sum(ctl.amount), 0) as target FROM adireksa_customer_target ct
            inner join adireksa_customer_target_line ctl on ctl.target_id = ct.id 
            where ct.id = {0} and ct.company_id = {1} and ctl.state = 'approve' and ctl.subtype = 'wilayah'""".format(group_id, self.company_id.id))
        tgt = cr.dictfetchone()
        target =  tgt.get('target') if tgt else 0.0
        customer_target_obj = self.env['adireksa.customer.target'].browse(group_id)
        wilayah_target = customer_target_obj.wilayah_target
        if target != wilayah_target:
            target = wilayah_target
        if revenue >= target :
            data_line = data.line_level_id.filtered(lambda x:x.customer_area_id == customer_target_obj.area_id)
            total = (data_line.formula / 100) * (revenue)
            ref = 'Omzet Periode %s (%s) Tahun %s Hit Target, Total Invoice: Rp. %s' % (
            title, month_range, year,'{:0,.0f}'.format(revenue))
            line_id.append(data.id)
        return math.ceil(total), ref, line_id
    ...
    def compute_cashback_in(self, data, group_id, frequency, revenue, target, status,dfrom,dto):

        if data.trigger == "sales_percent" and data.line_level_id:
            return self.check_frequency_percent_value(data, group_id, frequency, revenue, target, status,dfrom,dto)
        elif data.trigger == "sales_amount" and data.line_level_id:
            return self.check_frequency_amount_value(data, group_id, frequency, revenue, target, status,dfrom,dto)
        elif data.trigger == "revenue" :
            return self.check_frequency_revenue_value(data, group_id, frequency, revenue, target, status,dfrom,dto)

        elif data.trigger == "invoice":
            return self.check_frequency_invoice_value(data, group_id, frequency, revenue, target, status,dfrom,dto)

        elif data.trigger == "wilayah" :
            return self.check_frequency_wilayah_value(data, group_id, frequency, revenue, target, status,dfrom,dto)
        ...
        #         total = (data.formula / 100) * (revenue - target)
        #         # ref = data.name
        #         ref = 'Omzet Periode Annual (Januari s/d Desember) Hit Target, Total Invoice: Rp. %s' % (
        #             '{:0,.0f}'.format(revenue))
        #         line_id.append(data.id)
        # if frequency == '6month':
        #     actual_target = target
        #     if data.trigger == 'sales_percent':
        #         percent = (revenue /actual_target ) * 100
    ...
    def check_duplicated_cashback(self, freq, group_id, date,line):
        cr = self.env.cr
        ...
        cr.execute("""SELECT count(mcc.id) as total FROM automatic_cashback_lines acl
            ...
            and crl.rule_id = {2} and mcc.group_id = {3} and crl.trigger = '{4}' and crl.frequency = '{5}'
            GROUP BY mcc.id""".format(dfrom, dto, self.id, group_id, line.trigger , freq))
        res = cr.dictfetchone()
        ...
        cr.execute("""SELECT acl.id FROM automatic_cashback_lines acl
            ...
            and crl.rule_id = {2} and mcc.group_id = {3} and crl.trigger = '{4}' and crl.frequency = '{5}'
            GROUP BY acl.id""".format(dfrom, dto, self.id, group_id, line.trigger ,freq))
        rsc = cr.dictfetchall()
        if rsc:
            for rs in rsc:
    ...
    def cron_automate_cashback_in(self, frequency):
        ...
        if res:
            for rs in res:
                ...
                revenue, target, date, status,account,dfrom,dto = self.compute_revenue(group_id, cid, frequency)
                id_invoice = []
                cashback = ''
                if account :
                    for rec in account:
                        id_invoice.append(rec['id'])
                    cashback = self.env['cashback.invoice'].create({
                        'name':self.env['ir.sequence'].next_by_code('cashback.invoice'),
                        'company_id':self.env.company.id,
                        'invoice_ids' : id_invoice
                    })
                # if frequency in ['quarter', '6month']:
                total = 0.0
                ref = ''
                line_id = []
                if target > 0 or self.env.company.id == 2 :
                        for line in self.line_ids.filtered(lambda r: r.frequency == frequency).sorted(
                                key=lambda r: r.sequence):
                            amount, details, lid = self.compute_cashback_in(line, group_id, frequency, revenue, target,
                                                                        status,dfrom,dto)
                            total = amount if amount else 0
                            ref = details if amount else ''
                            line_id = lid if amount else line_id

                            check = self.check_duplicated_cashback(frequency, group_id, date,line)
                            if total > 0.0:
                ...
                # else:
                #     data = []
                #     for line in self.line_ids.filtered(lambda r: r.frequency == 'month').sorted(
                #             key=lambda r: r.sequence):
                #         data.append({
                #             'id': line.id,
                #             'name': line.name,
...
class CashbackRuleLine(models.Model):
    ...
    line_level_id = fields.One2many('cashback.rule.line.level','line_id')

    trigger = fields.Selection([
        ('revenue', 'Hit Target Revenue'),
        ('sales_percent', 'Hit Sales By Percent'),
        ('sales_amount', 'Hit Sales By Amount'),
        ('invoice', 'Pay Invoice'),
        ('wilayah', 'Wilayah'),
...
class CashbackRuleLineLevel(models.Model):
    _name = 'cashback.rule.line.level'
    _description = 'Cashback Rule Line Level'

    name = fields.Char(string="Name")
    line_id = fields.Many2one('cashback.rule.line')
    payment_id = fields.Many2one('account.payment.term')
    percent_hit = fields.Integer(string='Percent Hit(Dalam %)')
    amount_hit = fields.Integer(string='Amount Hit')
    formula = fields.Float(string='Formula (%)', default=0.0)
    days_before = fields.Integer(string="Days")
    trigger = fields.Selection([
        ('revenue', 'Hit Target Revenue'),
        ('sales_percent', 'Hit Sales By Percent'),
        ('sales_amount', 'Hit Sales By Amount'),
        ('invoice', 'Pay Invoice'),
        ('wilayah', 'Wilayah'),
    ], string='Trigger', default='revenue',related="line_id.trigger")
    customer_area_id = fields.Many2one('customer.area')


    @api.onchange('customer_area_id')
    def onchange_formula_percent(self):
        self.formula = self.customer_area_id.formula_percent