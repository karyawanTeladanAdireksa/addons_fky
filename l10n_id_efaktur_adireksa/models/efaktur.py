...
class Efaktur(models.Model):
    ...
    @api.model
    def pop_number(self, company_id=None):
        #range = self.search([('company_id', '=', company_id)], order="min ASC", limit=1)
        range = self.search([], order="min ASC", limit=1)
        if not range:
            return None
    ...
    def _compute_default(self):
        for record in self:
            ...
            max_available = int(self.env['l10n_id_efaktur.efaktur.range'].search([], order='max DESC', limit=1).max)
