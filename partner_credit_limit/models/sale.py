...
class SaleOrder(models.Model):
    ...
    def check_limit(self):
        ...
        today_dt = datetime.strptime(str(datetime.now().date()), DF).date()
        for line in movelines:
            if line.date_maturity < today_dt:
                credit += line.debit
                debit += line.credit
        available_credit = partner.available_credit + self.amount_total
        if (credit - debit + self.amount_total) > partner.credit_limit or self.amount_total > available_credit :
