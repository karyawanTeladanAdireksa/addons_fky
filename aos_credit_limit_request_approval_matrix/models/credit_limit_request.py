...
class CreditLimitRequest(models.Model):
    ...
    _inherit = ["credit.limit.request", "approval.matrix.mixin"]
    
    approver = fields.Text(compute="_compute_approver")
    approved_by_ids = fields.Many2many('res.users',compute="_compute_approver")
    minimum_approved = fields.Integer()
    ...
    def btn_approve(self):
        ...
        if self.approved:     
            self.with_context(allowed_company_ids=self.company_id.ids,cids=self.company_id.id).credit_amount_approve()
    ...
    def open_reject_message_wizard(self):
        ...
        context.update({'default_prefix_message':"<h4>Rejecting Credit Limit Request</h4>","default_suffix_action": "button_reject"}) #uncomment if need append context
        context.update({'active_id':self.id,'active_ids':self.ids,'active_model':'credit.limit.request'})
        res = {
            'name': "%s - %s" % (_('Rejecting Credit Limit Request'), self.name),
            'view_type': 'form',
            'view_mode': 'form',
