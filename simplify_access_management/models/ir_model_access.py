...
{resolution_info}""".format(
        ...
        try:
            ...
            if self.env.user.id and read_value:
                # EOI551 - if there is a cookie available, use that
                if request.httprequest.cookies.get('cids'):
                    req = str(request.httprequest.cookies.get('cids') and request.httprequest.cookies.get('cids').split(',')[0] or request.env.company.id)
                else:
                    req = request.env.company.id
                a = "select access_management_id from access_management_comapnay_rel where company_id = " + req
                self._cr.execute(a)
                a = self._cr.fetchall()
                if a:    
