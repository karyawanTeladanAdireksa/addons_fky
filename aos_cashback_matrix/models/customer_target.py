...
class CustomerTarget(models.Model):
    ...
    _inherit = ["adireksa.customer.target", "approval.matrix.mixin"]
    ...
    def btn_approve(self):
        ...
        if self.approved:     
            self.with_context(allowed_company_ids=self.company_id.ids,cids=self.company_id.id).action_approve()
    ...
    def open_reject_message_wizard(self):
        ...
        context.update({'default_prefix_message':"<h4>Rejecting CashBack</h4>","default_suffix_action": "button_reject"}) #uncomment if need append context
        context.update({'active_id':self.id,'active_ids':self.ids,'active_model':'adireksa.customer.target'})
        res = {
            'name': "%s - %s" % (_('Rejecting Cashback'), self.name),
            'view_type': 'form',
            'view_mode': 'form',
