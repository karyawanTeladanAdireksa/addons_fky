...
class customer_limit_wizard(models.TransientModel):
    ...
    def set_credit_limit_state(self):
        ...
        partner_id.sudo().credit_limit_on_hold = self.credit_limit_on_hold
        return True
    
    current_sale = fields.Float('Current Quotation')
    current_sale_day = fields.Char('Current Day Quot')
    
    exceeded_amount = fields.Float('Exceeded Amount')
    exceeded_day = fields.Char('Exceeded Day')
    
    credit = fields.Float('Total Receivable')
    credit_day = fields.Char('Days Receivable')
    
    partner_id = fields.Many2one('res.partner',string="Customer")
    credit_limit = fields.Float(related='partner_id.credit_limit',string="Credit Limit")
    credit_day_limit = fields.Integer(related='partner_id.credit_day_limit',string="Credit Day(s) Limit")
