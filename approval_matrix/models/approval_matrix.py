...
class ApprovalMatrix(models.Model):
    ...
    notification_activity = fields.Boolean('By Activity', default=True)
    notification_email = fields.Boolean('By Email')
    ...
    type = fields.Selection([('sequential','Sequential'), ('parallel','Parallel')], string='Type', default='parallel')#sequential, parallel
...
class ApprovalMatrixMixin(models.AbstractModel):
    ...
    approval_ids = fields.One2many('approval.matrix.document.approval', 'res_id', auto_join=True, groups="base.group_user", copy=False)
    ...
    def _compute_user_can_approve(self):
        for rec in self:
            rec.user_can_approve = any(rec.approval_ids.mapped('user_can_approve'))
...
class ApprovalMatrixDocumentApproval(models.Model):
    ...
    #     to_clean, to_do = self.env[self.res_model], self.env[self.res_model]
    #     # for approval in self:
    #         #print ('===_activity_update===',approval.res_model_id)
    #     note = _(
    #         '<b>APPROVAL REQUEST for %(model)s #%(number)s </b><br/>DOC. %(number)s need to review for your approval!',
    #         model=self.res_model_id.name,
    #         number=self.res_name
    #     )
    #     # if self.state == 'draft':
    #     #     to_clean |= self
    #     if len(self.approver_ids):
    #         users = self.approver_ids
    #         doc = self.env[self.res_model].browse(self.res_id)
    #         try:
    #             for user in users:
    #                 doc.activity_schedule(
    #                     'approval_matrix.mail_act_approval_matrix',
    #                     note=note,
    #                     user_id=user.id)
    #         except Exception as e:
    #             _logger.warning(_("Failed to send notification which model %s not applying activity.mixin") % (self.res_model,))
    #     # if to_clean:
    #     #     to_clean.activity_unlink(['approval_matrix.mail_act_leave_approval'])
    #     # if to_do:
    #     #     to_do.activity_feedback(['approval_matrix.mail_act_leave_approval'])
    ...
    def _activity_update(self):
        ...
        if len(self.approver_ids):
            # users = self.approver_ids
            doc = self.env[self.res_model].browse(self.res_id)
            # print ('-doc= state-',doc.state)
            if doc.state in ('draft', 'cancel'):
                to_clean |= doc
            elif doc.state == 'to approve':
                # if doc.activity_ids.filtered(lambda a: a.user_id == self.env.user):
                #     print ('---FEEDBACK-APPROVE---',doc.activity_ids)
                to_do |= doc
                # try:
                #     for user in users:
                #         doc.activity_schedule(
                ...
                #             user_id=user.id)
                # except Exception as e:
                #     _logger.warning(_("Failed to send notification wich model %s not applying activity.mixin") % (self.res_model,))
            elif doc.state == 'rejected':
                to_clean |= doc
        #print ('-to_clean-to_do-',to_clean,to_do)
        if to_clean:
            #HAPUS JIKA KLIK CANCEL ATAU DRAFT
            to_clean.activity_unlink(['approval_matrix.mail_act_approval_matrix'])
        if to_do:
            to_do.activity_feedback(['approval_matrix.mail_act_approval_matrix'])

    def _activity_create(self):
        # to_clean, to_do = self.env[self.res_model], self.env[self.res_model]
        note = _(
            '<b>APPROVAL REQUEST for %(model)s #%(number)s </b><br/>DOC. %(number)s need to review for your approval!',
            model=self.res_model_id.name,
        ...
        for rec in self:
            ...
            if len(rec.approver_ids):
                ...
                try:
                    ...
                    if rec.matrix_id.notification_activity:
                        rec._activity_update()
                        rec._activity_create()
                    if rec.matrix_id.notification_email:
                        doc.message_post(
    ...
    def _compute_user_can_approve(self):
        ...
        for rec in self:
            ...
            if self.env.user.id in rec.approver_ids.ids and \
                ...
                #update
                sequence = rec.approver_seq - 1
                if rec.approver_seq <= 1 :
                    res = True
                elif rec.approver_seq >= 2  :
                    last_matrix = rec.filtered(lambda x:x.approver_seq == sequence)
                    if  len(last_matrix.approved_by_ids) >= last_matrix.minimum_approved :
                #update
                        res = True
    ...
    def _validate_approving(self):
        ...
        #print ('=====all_has_rejected_by_user===',all_has_rejected_by_user)
        ...
        for appr in to_approve_rules:
            #print ('===approvals===',to_approve_rules)
            ...
            if appr.matrix_id.type == 'sequential' and len(approvals):
                ...
                to_approve_rules += approvals
        return to_approve_rules


    def approve(self):
        approvals = self._validate_approving()
        approvals.write({
