...
class CouponProgram(models.Model):
    ...
    invoice_count = fields.Integer(compute='_compute_order_count')

    # The api.depends is handled in `def modified` of `sale_coupon/models/sale_order.py`
    def _compute_order_count(self):
        for program in self:
            program.invoice_count = self.env['account.move.line'].sudo().search_count([('product_id', '=', program.discount_line_product_id.id)])

    def action_view_account_move(self):
        self.ensure_one()
        orders = self.env['account.move.line'].search([('product_id', '=', self.discount_line_product_id.id)]).mapped('move_id')
        return {
            'name': _('Invoice'),
            'view_mode': 'tree,form',
    ...
    def _check_promo_code(self, order, coupon_code):
        ...
        #     message = {'error': _('Global discounts are not cumulative.')}
            ...
            message = {'error': _('The reward products should be in the invoice lines to apply the discount.')}
    ...
    def _filter_on_mimimum_amount(self, order):
        ...
        for program in self:
            if program.reward_type != 'discount':
                ...
                lines = order.invoice_line_ids.filtered(lambda line:
    ...
    def _filter_programs_on_products(self, order):
        ...
        """
        order_lines = order.invoice_line_ids.filtered(lambda line: line.product_id) - order._get_reward_lines()
        products = order_lines.mapped('product_id')
        products_qties = dict.fromkeys(products, 0)
        for line in order_lines:
            products_qties[line.product_id] += line.quantity
    ...
    def _filter_not_ordered_reward_programs(self, order):
        ...
        order_products = order.invoice_line_ids.product_id
        for program in self:
