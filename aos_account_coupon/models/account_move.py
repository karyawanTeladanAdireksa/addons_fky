...
class AccountMove(models.Model):
    ...
    total_reward = fields.Float(string="Total Reward",compute="_compute_reward")

    @api.depends('invoice_line_ids.price_subtotal')
    def _compute_reward(self):
        for rec in self:
            rec.total_reward = 0
            if rec.invoice_line_ids.filtered(lambda x:x.is_reward_line == True):
                rec.total_reward = sum(rec.invoice_line_ids.filtered(lambda x:x.is_reward_line == True).mapped('price_unit'))
    ...
    def recompute_coupon_lines(self):
        for order in self:
            ...
            # order.with_context(check_move_validity=False)._update_existing_reward_lines()
            self.with_context(check_move_validity=False)._onchange_currency()

    @api.returns('self', lambda value: value.id)
    def copy(self, default=None):
        order = super(AccountMove, self).copy(default)
    ...
    def action_draft(self):
        res = super(AccountMove, self).action_draft()
        self.generated_coupon_ids.write({'state': 'reserved'})
        return res

    def _get_reward_lines(self):
        self.ensure_one()
        return self.invoice_line_ids.filtered(lambda line: line.is_reward_line)

    def _is_reward_in_order_lines(self, program):
        self.ensure_one()
        order_quantity = sum(self.invoice_line_ids.filtered(lambda line:
            line.product_id == program.reward_product_id).mapped('quantity'))
    ...
    def _get_reward_values_product(self, program):
        price_unit = self.invoice_line_ids.filtered(lambda line: program.reward_product_id == line.product_id)[0].price_unit

        order_lines = (self.invoice_line_ids - self._get_reward_lines()).filtered(lambda x: program._get_valid_products(x.product_id))
        max_product_qty = sum(order_lines.mapped('quantity')) or 1
        total_qty = sum(self.invoice_line_ids.filtered(lambda x: x.product_id == program.reward_product_id).mapped('quantity'))
    ...
    def _get_paid_order_lines(self):
        ...
        return self.invoice_line_ids.filtered(lambda x: not x._is_not_sellable_line() or x.product_id in free_reward_product)

    def _get_base_order_lines(self, program):
        """ Returns the sale order lines not linked to the given program.
        """
        return self.invoice_line_ids.filtered(lambda x: not x._is_not_sellable_line() or (x.is_reward_line and x.product_id != program.discount_line_product_id))
    ...
    def _get_cheapest_line(self):
        ...
        return min(self.invoice_line_ids.filtered(lambda x: not x._is_not_sellable_line() and x.price_unit > 0), key=lambda x: x['price_unit'])

    def _get_reward_values_discount_percentage_per_line(self, program, line):
        exist_discount_item = False
        if program.discount_apply_on == 'specific_products':
            have_reward = self.line_ids.filtered(lambda x:x.is_reward_line == True)
            exist_discount_value = 0
            if have_reward:
                coupon = program.search([('name','in',have_reward.mapped('coupon_name'))])
                exist_program = program.search([('discount_specific_product_ids','in',line.product_id.id)]).filtered(lambda x:x.id in coupon.ids)
                for reward in exist_program:
                    free_product_lines = self.env['coupon.program'].search([('reward_type', '=', 'product'), ('reward_product_id', 'in', reward.discount_specific_product_ids.ids)]).mapped('discount_line_product_id')
                    exist_discount_item = self.line_ids.filtered(lambda x: x.product_id in (reward.discount_specific_product_ids | free_product_lines)).filtered(lambda x:x.name == line.name)
                    exist_discount_value += exist_discount_item.price_subtotal * (reward.discount_percentage / 100)
        if exist_discount_item:   
            discount_amount = (line.price_subtotal - exist_discount_value) * (program.discount_percentage / 100)
        else:
            discount_amount = line.price_subtotal * (program.discount_percentage / 100)

        discount_amount = discount_amount + (discount_amount * sum(line.tax_ids.mapped('amount')))/100
        return discount_amount

    def _get_max_reward_values_per_tax(self, program, taxes):
        lines = self.invoice_line_ids.filtered(lambda l: l.tax_ids == taxes and l.product_id != program.discount_line_product_id)
    ...
    def _get_reward_values_fixed_amount(self, program):
        ...
        if program.discount_line_product_id.taxes_id:
            ...
            discount_amount = min(
                sum(lines.mapped(lambda l: l.price_unit * l.quantity)), discount_amount
            )
            return [{
                ...
                'is_reward_line': True,
                'tax_ids': [(4, tax.id, False) for tax in line_taxes],
            }]

        lines = self._get_paid_order_lines()
        # Remove Free Lines
        lines = lines.filtered('price_unit')
        reward_lines = {}

        tax_groups = set([line.tax_ids for line in lines])
        ...
        for tax_ids in sorted(tax_groups, key=lambda tax_ids: max_discount_per_tax_groups[tax_ids], reverse=True):
            ...
            curr_lines = lines.filtered(lambda l: l.tax_ids == tax_ids)
            lines_price = sum(curr_lines.mapped(lambda l: l.price_unit * l.quantity))
            ...
            reward_lines[tax_ids] = {
                ...
                'product_uom_id': program.discount_line_product_id.uom_id.id,
                'is_reward_line': True,
                'tax_ids': [(4, tax.id, False) for tax in tax_ids],
                }
        return reward_lines.values()
    ...
    def _get_reward_values_percentage_amount(self, program):
        ...
        self.invoice_line_ids.filtered(lambda l: l.product_id in fixed_price_products).write({'price_unit': 0})

        reward_dict = {}
        lines = self._get_paid_order_lines()
        amount_total = sum([any(line.tax_ids.mapped('price_include')) and line.price_total or line.price_subtotal
                            for line in self._get_base_order_lines(program)])
        if program.discount_apply_on == 'cheapest_product':
            line = self._get_cheapest_line()
            if line:
                discount_line_amount = min(line.price_unit * (program.discount_percentage / 100), amount_total)
    ...
    def _get_reward_line_values(self, program):
        ...
        seq = max(self.invoice_line_ids.mapped('sequence'), default=10) + 1
        for value in values:
            value['sequence'] = seq
        return values

    def _create_reward_line(self, program):
        self.write({'invoice_line_ids': [(0, False, value) for value in self._get_reward_line_values(program)]})
    ...
    def _send_reward_coupon_mail(self):
        template = self.env.ref('aos_account_coupon.mail_template_invoice_coupon', raise_if_not_found=False)
        if template:
            for order in self:
    ...
    def _create_new_no_code_promo_reward_lines(self):
        ...
        for program in programs:
            ...
            if not error_status.get('error'):
                ...
                elif program.discount_line_product_id.id not in self.invoice_line_ids.mapped('product_id').ids:
                    self.write({'invoice_line_ids': [(0, False, value) for value in self._get_reward_line_values(program)]})
                    # self.with_context(check_move_validity=False)._compute_amount_residual()
                    self.with_context(check_move_validity=False)._onchange_currency()
    ...
    def _update_existing_reward_lines(self):
        ...
        def update_line(order, lines, values):
            ...
            values['sequence'] = max(order.invoice_line_ids.mapped('sequence')) + 1

            # Check commit 6bb42904a03 for next if/else
            # Remove reward line if price or qty equal to 0
            if values['quantity'] and values['price_unit']:
                lines.write(values)
            else:
        ...
        for program in applied_programs.sorted(lambda ap: (ap.discount_type == 'fixed_amount', ap.discount_apply_on == 'on_order')):
            ...
            lines = order.invoice_line_ids.filtered(lambda line: line.product_id == program.discount_line_product_id)
            if program.reward_type == 'discount':
                lines_to_remove = lines
                lines_to_add = []
                lines_to_keep = []
                ...
                for value in values:
                    ...
                    for line in lines:
                        ...
                        if not len(set(line.tax_ids.mapped('id')).symmetric_difference(set([v[1] for v in value['tax_ids']]))):
                            ...
                            if program.program_type == 'promotion_program'and program.promo_code_usage == 'no_code_needed' :
                                amount_total = (sum(self.invoice_line_ids.filtered(lambda x:x.id != line.id).mapped('price_subtotal')) * (program.discount_percentage /100))
                                amount_total = amount_total + (amount_total * 11)/100
                                line.update({'price_unit':-amount_total})
                                value['price_unit'] = -amount_total
                                lines_to_update.append((line,value))
                ...
                order.write({'invoice_line_ids': line_update})
                if lines_to_update :
                    for rec in lines_to_update:
                        try :
                            rec[0].price_unit = rec[1]['price_unit']
                        except :
                            continue
    ...
    def _remove_invalid_reward_lines(self):
        ...
        invalid_lines = order.invoice_line_ids.filtered(lambda line: line.is_reward_line and line.product_id.id not in reward_product_ids)
        if programs_to_remove:
            ...
            if product_ids_to_remove:
                invalid_lines |= order.invoice_line_ids.filtered(lambda line: line.product_id.id in product_ids_to_remove)
    ...
    def update_prices(self):
        ...
        super().update_prices()
        if any(line.is_reward_line for line in self.invoice_line_ids):
            self.recompute_coupon_lines()
...
class AccountMoveLine(models.Model):
    ...
    def unlink(self):
        ...
        for line in self.filtered(lambda line: line.is_reward_line):
            ...
            if related_program:
                line.move_id.no_code_promo_program_ids -= related_program
                line.move_id.code_promo_program_id -= related_program
                related_program_lines |= line.move_id.invoice_line_ids.filtered(lambda l: l.product_id.id == related_program.discount_line_product_id.id) - line
        return super(AccountMoveLine, self | related_program_lines).unlink()
    
    

    def _compute_tax_id(self):
        reward_lines = self.filtered('is_reward_line')
        super(AccountMoveLine, self - reward_lines)._compute_tax_id()
        ...
        for line in reward_lines:
            ...
            fpos = line.move_id.fiscal_position_id or line.move_id.fiscal_position_id.get_fiscal_position(line.order_partner_id.id)
    ...
    def modified(self, fnames, *args, **kwargs):
        super(AccountMoveLine, self).modified(fnames, *args, **kwargs)
        if 'product_id' in fnames:
            Program = self.env['coupon.program'].sudo()
            field_order_count = Program._fields['invoice_count']
