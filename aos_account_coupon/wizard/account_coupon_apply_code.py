...
class SaleCouponApplyCode(models.TransientModel):
    _name = 'account.coupon.apply.code'
    _rec_name = 'coupon_code'
    _description = 'Account Coupon Apply Code'
    ...
    def process_coupon(self):
        ...
        """
        sales_order = self.env['account.move'].browse(self.env.context.get('active_id'))
        ...
        if program:
            ...
            if not error_status:
                if program.promo_applicability == 'on_next_order':
                    ...
                    order_line_count = len(order.invoice_line_ids)
                    order._create_reward_line(program)
                    if order_line_count < len(order.invoice_line_ids):
                        order.code_promo_program_id = program
        else:
            coupon = self.env['coupon.coupon'].search([('code', '=', coupon_code)], limit=1)
            if coupon:
                error_status = coupon._check_coupon_code(order.invoice_date, order.partner_id.id, order=order)
                if not error_status:
